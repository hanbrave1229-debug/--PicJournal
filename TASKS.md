...
#### [x] P2-1 已打标统计
- 后端 `GET /photos/stats` 返回 `{total, tagged, untagged, videos, face_analyzed}`
- Settings.vue 打标区上方增加统计数字卡 + 进度条，打标完成后自动刷新

#### [x] P2-2 日记时间线视图
- 后端 `GET /diaries/list?page=1&page_size=20` 返回分页日记（最新优先）
- Diary.vue 右上角加日历/列表视图切换，卡片含封面图、心情 emoji、内容预览，支持加载更多

#### [x] P2-3 导入历史管理
- 后端 `GET /import/history` 返回 import_dir 路径下的扫描任务列表
- Settings.vue 扫描 tab 底部新增"导入历史"表格
